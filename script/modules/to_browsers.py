...
class To_browsers:
	...
	def create_proj_pages2(self):
		...
		for proj in self.active_projs:
			...
			<tr><td><strong>Comments: </strong></td>					<td>{8}</td></tr>
			<tr><td><strong>Details: </strong></td>					<td>{9}</td></tr>
			</table>\n""".format(proj, proj_latlon, proj_area, proj_sgr, proj_surveyed, proj_start_date, proj_last_date, proj_assessors, proj_comments, proj_detail)
...
def ecosite_to_html_table(ecosite, title='MNRF Ecosite Moisture'):
	""" an example of ecosite = {'fresh': 66.7, 'moist': 16.7, 'dry': 16.7}
	"""
	html = ''
	enough_data = True
	if len(ecosite) == 0:
		html += "\n<p>Ecosite has not been evaluated</p>\n"
		enough_data = False
	else:
		header = list(ecosite.keys())
		html += """\n<br><br>
			<table id="striped">
				<caption><strong>%s</strong>
				</caption>
		...
		html += "<tr>"		
		for head in header:
			html += "<td>{0} %</td>".format(ecosite[head])
		html += "</tr>\n</table><br>\n"
	return [enough_data, html]
