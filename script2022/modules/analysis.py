...
class Run_analysis:
	...
	def define_attr_names(self):
		...
		self.p_so_reason_sum = 'site_occupancy_reason_summary' # new in 2022. {'Skid Trail': 35.4, 'Not FTG': 21, ...}
	...
	def summarize_clusters(self):
		...
		for silvsys, cluster_in_dict in {'CC': self.cc_cluster_in_dict, 'SH': self.sh_cluster_in_dict}.items():
			for cluster in cluster_in_dict:
				...
				eco_nutri = cluster['NutrientEcosite'] # eg. Poor, Very Poor, Rich...
...
# eg. c_sharepoint_photopath = same format as c_local_sync_photopath but with url of sharepoint.
	...
	def summarize_projects(self):
		...
		for prj in self.prj_shp_in_dict:
			...
			# site (un)occupancy reason summary
			so_reason_sum = {} # new in 2022. {'Skid Trail': 35.4, 'Not FTG': 21, ...}
			so_reason_all = [] # ['Skid Trail','Skid Trail','Not FTG','Wet',...]
			for clus, val in so_reason.items():
				for plotnum, reason in val.items():
					if reason != '': so_reason_all.append(reason)
			so_reason_all_len = len(so_reason_all) # total number of occurrences of unoccupied reasons					
			so_reason_uniq = list(set(so_reason_all)) # get the list of unique values
			for u_reason in so_reason_uniq:
				match = [i for i in so_reason_all if i==u_reason]
				reason_percent = round(float(len(match))*100/so_reason_all_len,2)
				so_reason_sum[u_reason] = reason_percent
			record[self.p_so_reason_sum] = so_reason_sum
